...
//
// A cold path 'function' is a block of code that
// the hpcfnbounds detector thinks is an independent function.
// The block of code, however, is not treated as a function. Instead of
// being called, the code block is conditionally branched to from the 'parent' function.
// (it is not 'called' very often, so we call it cold path code).
// Furthermore a cold path function does not 'return', but rather jumps back to the
// instruction just after the conditional branch.
//
// These routines take care of detecting a cold path 'function', and updating the
// intervals of the cold path code.
//
