...
int main() {
  ...
  // Find a suitable platform and device, one that has an actual GPU attached
  {
    ...
    if (err != CL_SUCCESS) handleCLError(err, 77);  // OpenCL not actually available

    cl_platform_id* plats = malloc(num_plats * sizeof(cl_platform_id));
    err = clGetPlatformIDs(num_plats, plats, NULL);
    if (err != CL_SUCCESS) handleCLError(err, 1);
  ...
  if (err != CL_SUCCESS) {
    char log[4096] = {0};
    size_t logsz;
    cl_uint xerr = clGetProgramBuildInfo(program, device_id, CL_PROGRAM_BUILD_LOG, sizeof log, log, &logsz);
    if(xerr != CL_SUCCESS) {
