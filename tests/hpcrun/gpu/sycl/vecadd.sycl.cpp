...
int main(int argc, char* argv[]) {
  ...
  // Copy the data in
  SYCL_CALL( q.memcpy(d_a, a.data(), a.size() * sizeof(decltype(a)::value_type)).wait(), "Failed to memcpy a -> d_a: " );
  SYCL_CALL( q.memcpy(d_b, b.data(), b.size() * sizeof(decltype(b)::value_type)).wait(), "Failed to memcpy b -> d_b: " );
  ...
  SYCL_CALL( q.memcpy(c.data(), d_c, c.size() * sizeof(decltype(c)::value_type)).wait(), "Failed to memcpy d_c -> c: " );
