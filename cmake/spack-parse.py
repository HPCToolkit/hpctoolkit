...
spack = subprocess.run([sys.argv[1], 'find', '--format',
                        '{prefix}///{name}{@version} /{hash:7}\n'] + sys.argv[2:],
...
if spack.returncode != 0:
  ...
  spack = subprocess.run([sys.argv[1], 'find', sys.argv[2]],
                         capture_output=True, text=True)
  if spack.returncode == 0:
...
lines = [x.strip() for x in spack.stdout.splitlines()]
...
# Print out the data for the root that remains
prefix, spec = lines[0].split('///')
print(prefix)
print(spec)
