...
static void
csprof_take_profile_sample(csprof_state_t *state, int metric_id, 
			   struct ucontext *ctx)
{
...
#endif
    //    if(csprof_sample_callstack(state,context,metric_id,1) == CSPROF_OK) {
    if(csprof_sample_callstack(state,ctx,metric_id,1) == CSPROF_OK) {
#ifdef USE_TRAMP
...
void csprof_sample_event(void *context, int metric_id)
{
  ...
  if (!sigsetjmp(it->jb,1)){
    ...
    if(state != NULL) {
      csprof_take_profile_sample(state, metric_id, ctx);
