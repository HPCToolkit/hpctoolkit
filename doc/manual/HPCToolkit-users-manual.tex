...
Sampling enables fine-grain measurement and attribution of costs in both serial and parallel programs. For parallel programs running on a cluster or supercomputer, HPCToolkit can measure the how often threads or compute nodes are idle, working, or communicating. To obtain detailed information about a program's computation performance, one can collect samples using the performance monitoring units built into modern processors  to measure metrics such as operation counts, pipeline stalls, cache misses, and cache-to-cache transfers between processor sockets.
