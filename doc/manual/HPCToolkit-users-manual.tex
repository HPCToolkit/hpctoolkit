...
\end{verbatim}
\end{quote}
\hpclink{}'s \mytt{--help} option gives a list of variables that affect monitoring.
...
\item
Scalability bottlenecks in parallel codes can be pinpointed by differential anal
ysis of two profiles with different degrees of parallelism (Section~\ref{sec:effective-performance-analysis:scalability}).
...
\begin{quote}
  ...
  \verb|hpcviewer hpctoolkit-database|
\end{quote}
...
2. use the hpclink binary with 
  ...
  export HPCRUN_EVENT_LIST="WALLCLOCK@0" or HPCRUN_EVENT_LIST="NONE"
  export MONITOR_DEBUG=1
