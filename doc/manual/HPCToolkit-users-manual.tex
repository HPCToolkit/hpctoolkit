...
This section lists some of known issues and the workaround. 
Other known issues can be seen in the github issues:
\begin{itemize}
 \item For hpctoolkit in general: \url{https://github.com/HPCToolkit/hpctoolkit/issues}
 \item For hpcviewer: \url{https://github.com/HPCToolkit/hpcviewer.e4/issues}
\end{itemize}

\section{\hpcrun{} has been observed to crash when measuring many threads and processes using ``{\tt -e cycles}" }

\paragraph{Affected architectures:}  any CPU 

\paragraph{Description:} Low sampling periods or frequency-based sampling on cycles (the default) on large numbers of processes seems to expose unknown race conditions that can cause an application to crash during measurement.

\paragraph{Workaround:} Using a large fixed sampling period, e.g. 10 million, may avoid this issue. Sampling using CPUTIME and REALTIME appears a more stable alternative. 

\paragraph{Development Plan:} Identify race conditions present and develop strategies to avoid them.

\section{After analyzing an execution traces  \hpcprofmpi{}, \hpcviewer{} may report invalid CPID (call path id) values when displaying traces}

\paragraph{Affected architectures:}  any CPU 


\paragraph{Description:} HPCToolkit assumes that the only call paths that appear in traces are leaves in a calling context tree. When analyzing traces with \hpcprofmpi{}, a strategy used to handle interior nodes that may have been traced doesn't work when analysis of traces is performed with \hpcprofmpi{} using multiple processes. 
...
\paragraph{Development Plan:} A forthcoming version of \hpcprofmpi{} will not require that any node appearing in a trace is a leaf in a calling context tree.
