...
\title{\HPCToolkit{} User's Manual\\[.5in]Version 2021.02.15}
%\subtitle{}
...
\paragraph{Affected architectures:} {\tt x86\_64} CPUs 

\paragraph{Description:} We observed a problem using Linux  {\tt perf\_events} to measure CPU performance using hardware performance counters on one {\tt x86\_64} cluster at Sandia. Another system at Sandia with what appears to be the same kernel and configuration works just fine. 

\paragraph{Workaround:} On the problematic system at Sandia, we found that measuring using HPCToolkit's PAPI interface atop Linux  {\tt perf\_events} worked even though using HPCToolkit directly atop Linux  {\tt perf\_events} yielded no measurement data. Additionally, sampling using CPUTIME and REALTIME may be suitable alternatives to measuring {\tt cycles}.

\paragraph{Development Plan:} Identify the difference between how PAPI and HPCToolkit use Linux {\tt perf\_events}.
