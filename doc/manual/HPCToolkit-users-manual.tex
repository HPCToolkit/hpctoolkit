...
\section{How do I handle error \texttt{CUPTI\_ERROR\_NOT\_INITIALIZED}?}
\label{sec:troubleshooting:cupti}

\hpcrun{} uses CUPTI to monitor GPU computation on NVIDIA GPUs. In our experience, this error 
is typically caused by incompatible versions between CUPTI and the CUDA driver.
Based on CUPTI documentation from NVIDIA, newer versions of CUPTI may be incompatible with older versions of CUDA driver.
Therefore, we recommend trying an older version of CUPTI or upgrading the CUDA driver when encountering error \texttt{CUPTI\_ERROR\_NOT\_INITIALIZED}.
