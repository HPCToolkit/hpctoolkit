...
\title{\HPCToolkit{} User's Manual\\[.5in]Version 2021.10.15}
%\subtitle{}
...
information about files, procedures, loops, and inlined code.
...
\HPCToolkit{} focuses its efforts on recovering procedures, inlined functions and templates, as well as
loop nests, the most important elements of source code structure.
...
Finally, one explores a performance database with \HPCToolkit{}'s graphical user interface: \hpcviewer{} which presentes
...
Typically, \hpcstruct{} is launched without any options, with an argument that is a \emph{measurements directory}.
It recovers the names of all shared libraries and GPU binaries invoked by the application.
It processes all of them, and puts links to the shared libaries and copies of the GPU binaries into that
directory.
It also puts the structure information for all of them into the \emph{measurements directory}.
The structure information includes its loop nesting structure, and inlining information.

By default, \hpcstruct{} processes multiple objects concurrently, and processes any large objects
using internal parallelism.

To recover static program structure for a single binary \texttt{bin}, use the command:
\begin{quote}
  \verb|hpcstruct bin|
\end{quote}
This command analyzes the binary and saves this information in a file named \texttt{bin.hpcstruct}.
...
In most respects, \hpcprof{} and \hpcprof-mpi{} are semantically idential.
