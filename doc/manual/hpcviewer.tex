...
It is sometimes useful to omit uninterested nodes in a CCT so that users can just focus on important parts of the code. 
For instance, users may want to hide all nodes associated with OpenMP runtime to better , 
and just show all nodes and metrics from their own application. 
...
Nodes \texttt{C} and its children are elided, and their exclusive cost is augmented to node \texttt{A} which is the parent of node \texttt{C}.}
 ...
 Each node is attributed with two boxes on its right.  The left box signifies the node's inclusive cost, while the right box signifies the exclusive cost.}
  \label{fig:filter}
\end{figure}
\begin{figure}
 \centering
...
If the checkbox is checked, is signifies the filter is enabled; otherwise the filter is disabled.
