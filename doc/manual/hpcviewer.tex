...
It is sometimes useful to omit uninterested nodes in a CCT so that users can just focus on the important parts of the code. 
For instance, users may need to hide all call chains within an OpenMP runtime, and just show all nodes and metrics from their own application. 
For this purpose, \hpcviewer{} provides \emph{filtering} to elide nodes that match a certain filter patterns.
...
\begin{figure}
  ...
  \begin{subfigure}{.45\textwidth}
    ...
    \caption{Example of a CCT tree. \texttt{Main} is the root of the tree, and has inclusive cost 9 and exclusive cost 1.}
...
The exclusive cost of node \texttt{C} is also augmented to node \texttt{A}.}
  ...
  \begin{subfigure}{.45\textwidth}
    ...
    \caption{The result of applying \emph{Children only} filter. % on node \texttt{C} against the CCT from Figure~\ref{fig:filter-cct}.
The children of node \texttt{C} (nodes \texttt{D} and \texttt{E}) are elided, and their exclusive cost is augmented to node \texttt{C}. }
  ...
  \begin{subfigure}{.45\textwidth}
    ...
    \caption{The result of applying \emph{self and children} filter. % on node \texttt{C} against the CCT from Figure~\ref{fig:filter-cct}. }
Nodes \texttt{C} and its children are elided, and their exclusive cost is augmented to node \texttt{A} which is the parent of node \texttt{C}.}
  ...
  \caption{Example of applying three different types of filter on node \texttt{C} against the tree from Figure~\ref{fig:filter-cct}. Each node is attributed with two boxes on its right. 
The leftmost box signifies the node's inclusive cost, while the right box signifies the exclusive cost.}
  \label{fig:filter}
\end{figure}
\begin{figure}
 \centering
...
\paragraph{Self and children}: This filter elides both the matched node and all its children. 
...
On \hpcviewer{}, the filter feature can be accessed by clicking the menu "Filter" and then submenu "Show filter property" which then will show the filter property window as shown in Figure~\ref{fig:filter-window}.
Users can specify multiple filter patterns and enable/disable some of them by clicking into the checkbox on the left of the pattern.
