...
\subsection{Table of thread-level metrics}
\label{sec:thread-level-table}

This Section describes \hpcviewer's feature to view the CCT of a certain threads (or processes).
...
On the other hand, to select a group of threads, you need to use the thread selection window by clicking \includegraphics[scale=.6]{fig/hpcviewer-thread.png} button from the calling-context view.
...
Hence, to specify just a main thread (thread zero), you can type '.0' on the filter, and the view only list threads 0 (such as 1.0, 2.0, 3.0 ....).
...
\label{sec:filter}
Occasionally, It is useful to omit uninterested nodes in the CCT so that you can focus on important parts. 
...
\paragraph{Descendants only}: This filter elides only the children of the matched node, while the matched node itself is not removed. 
...
\paragraph{Self and descendants}: This filter elides both the matched node and all its children. 
...
You have to be careful when using filter feature since it can result different the tree, thus affects different interpretation of performance analysis.
If the filtered nodes are children of a ``fake'' procedures (such as \texttt{<program root>} and \texttt{<thread root>}), the exclusive metrics in callers view and flat view can be misleading. 
