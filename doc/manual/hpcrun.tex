...
To force HPCToolkit to use PAPI (assuming it’s available) instead of \perfevents{}, 
...
“papi::”. For instance it is sufficient to specify \verb|PAPI_TOT_CYC| event
...
One can a different default rate for frequency-based sampling using the \verb|-c| option. The command below will sample \verb|CYCLES| at 200 samples per second and \verb|INSTRUCTIONS| at 200 samples per second:

\begin{quote}
\begin{verbatim}
hpcrun -c 200 -e CYCLES -e INSTRUCTIONS …
\end{verbatim}
\end{quote}
