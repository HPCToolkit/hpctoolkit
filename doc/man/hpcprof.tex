...
\Prog{hpcprof} analyzes call path profile performance measurements and attributes them to static source code structure.
...
\Prog{hpcprof} analyzes call path profile performance measurements, attributes them to static source code structure, and generates an Experiment database for use with \HTMLhref{hpcviewer.html}{\Cmd{hpcviewer}{1}}.
For analyzing and attributing the performance of large-scale executions, use \HTMLhref{hpcprof-mpi.html}{\Cmd{hpcprof-mpi}{1}}, a parallel version of \Prog{hpcprof}.

\Prog{hpcprof} expects a list of measurement-groups, where a group is a call path profile directory or an individual profile file.
...
\Arg{metric} is one of the following:
  \begin{itemize}
  ...
  \item[thread] Show only thread metrics
  \end{itemize}
Default is 'sum'.
(\Cmd{hpcprof-mpi}{1} always computes 'sum'.)
...
\subsection{Options: Statistics}

\begin{Description}
\item[\OptArg{--stats}{ }]
Compute extra summary statistics for each measured metric. 
Without using this flag, hpcprof will compute only a Sum for each measured metric. 
Specifying this flag will also cause hpcprof to compute 
Mean, StdDev (standard deviation), CoefVar (coefficient of variation), Min, Max 
for each metric as well.
\end{Description}
