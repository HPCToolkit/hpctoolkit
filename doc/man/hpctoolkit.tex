...
\begin{enumerate}
\item \textbf{Measure}.  Profile with \Cmd{hpcrun}{1}.  Assume we wish to use two different sets of events and that the results are placed in \File{profile-file-1} and \File{profile-file-2}.
\begin{verbatim}
...
Create an Experiment database using \Cmd{hpcquick}{1}, a front-end to \Cmd{hpcview}{1}.  Assume the Experiment database is located in \File{experiment-db}.
\begin{verbatim}
...
\begin{enumerate}
\item \textbf{Create derived metrics}.  Add COMPUTE metrics to the \Cmd{hpcview}{1} configuration file generated by \Cmd{hpcquick}{1}, typically stored in \File{hpcquick.xml}.  The \Cmd{hpcview}{1} man page contains some examples. 

\item \textbf{Correlate}. Rerun \Cmd{hpcview}{1} to create a new Experiment database.  Assume that it is located in \File{experiment-db-1}
\begin{verbatim}
