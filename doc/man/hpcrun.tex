...
Collect comprehensive operation-level data, including timing of GPU kernel invocations,
memory copies (implicit and explicit), driver and runtime activity, and overhead.
If the optional \Arg{pc} is used, PC sampling on the GPU is used inaddition to the operation-level data.
...
Normally as \Prog{hpcrun} collects data against full callstacks at the time of each event.
If this option is given, \Prog{hpcrun} will skip unwinding, and generate data with only
the leafPC's.
