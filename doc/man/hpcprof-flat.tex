...
\Prog{hpcprof-flat} correlates dynamic (`flat') profile metrics with static source code structure.  See \HTMLhref{hpctoolkit.html}{\Cmd{hpctoolkit}{1}} for an overview of \textbf{HPCToolkit}.
...
\Prog{hpcprof-flat} \oOpt{options} \Arg{config-file}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Description}

\Prog{hpcprof-flat} correlates dynamic profiling metrics with static source code structure and (by default) generates an Experiment database for use with \HTMLhref{hpcviewer.html}{\Cmd{hpcviewer}{1}}.
\Prog{hpcprof-flat} is driven by the configuration file \Arg{config-file}, which amongh other things, may contain user defined derived metrics.

The easiest way to create a configuration file is to first run \Prog{hpcprof-flat} and edit the configuration file it generates.
...
A \Prog{hpcprof-flat} configuration file is an XML document of type \texttt{HPCVIEW}.
...
The two metric types are
  \begin{itemize}
  \item \textbf{Native or FILE metrics}.  This metric is read from a file which can be of type HPCRUN (from \HTMLhref{hpcrun.html}{\Cmd{hpcrun}{1}}) or PROFILE (from \Cmd{hpcproftt}{1}).
...
There is one important difference in how each type is handled.  HPCRUN files are correlated to source code using the object code addresses annotations of STRUCTURE files; thus, they require valid STRUCTURE information.  In contrast, PROFILE files are correlated by source line level information.
  ...
  \item \textbf{Derived or COMPUTE metrics.} Derived metrics are specified by a \texttt{COMPUTE} element containing a MathML equation in terms of metrics defined earlier in the HPCVIEW document.  \Prog{hpcprof-flat} supports the following operands
  \begin{itemize}
