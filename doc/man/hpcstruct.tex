...
measurements.
\Prog{hpcstruct} is usually run against an application's HPCToolkit \Arg{measurement-directory} directing
it to analyze all CPU and GPU binaries recorded within it.
Although no normally needed, one can apply \Prog{hpcstruct} to recover program
structure for an individual CPU or GPU binary.

When \Prog{hpcprof} is applied to a \Arg{measurement-directory} that contains program structure files,
...
\Prog{hpcstruct} emits a message for each binary processed saying one of "Added to cache",
...
If the \Arg{--nocache} option will not use the cache, even if the environment variable is set, and the message
will say "(Cache disabled by user)".
...
threshold (see the \Arg{--psize} option and its default) are analyzed using
...
When analyzing a single CPU or GPU binary \Arg{b}, \Prog{hpcstruct} only writes its results to the file
'basename(\Arg{b}).hpcstruct' in the current directory, and does not link to the binary.
...
When processing a \Arg{measurement directory},large binaries (size greater than \Arg{psize})
are processed first, one at a time, with each using
\Arg{num} OpenMP threads in \Prog{hpcstruct}.
Following that, all other binaries are processes concurrently with \Arg{num} 
processes running, each using one OpenMP thread.
When processing a single binary, \Arg{num} OpenMP threads are used.
...
Indicate that the invocation of \Prog{hpcstruct} comes from the Makefile generated
by processing a \Arg{measurements-directory}.
