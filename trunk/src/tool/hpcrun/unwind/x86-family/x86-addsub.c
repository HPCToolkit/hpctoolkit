	       ...
	       bool *bp_frames_found)
{
  ...
  if ((op0_name == XED_OPERAND_REG0) &&
	...
	*bp_frames_found = true;
      ...
      if (immedv > 0) {
	if (highwatermark->type != HW_SPSUB && highwatermark->type != HW_BPSAVE_AFTER_SUB && highwatermark->type != HW_BPHOSED) {
	  //-------------------------------------------------------------------------
	...
	next = new_ui(ins + xed_decoded_inst_get_length(xptr), RA_BP_FRAME, 
		      current->sp_ra_pos, current->bp_ra_pos, current->bp_status, 
		      current->sp_bp_pos, current->bp_bp_pos, current);
	*bp_frames_found = true;
      }
    }
  }
