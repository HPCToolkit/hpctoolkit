...
should add the {\tt HPCTOOLKIT} environment variable to your launch
script.  Set {\tt HPCTOOLKIT} to the top-level \HPCToolkit{} install
prefix (the directory containing the {\tt bin}, {\tt lib} and {\tt
libexec} subdirectories) and export it to the environment.  This is
...
If {\tt HPCTOOLKIT} is not set, you may see errors such as the
...
The problem is that the Cray job launcher copies the {\tt hpcrun}
script to a directory somewhere below {\tt /var/spool/alps/}\ and runs
it from there.  By moving {\tt hpcrun} to a different directory, this
breaks {\tt hpcrun}'s method for finding its own install directory.
The solution is to add {\tt HPCTOOLKIT} to your environment so that
{\tt hpcrun} can find its install directory.

Your system may have a module installed for {\tt hpctoolkit} with the
correct settings for {\tt PATH}, {\tt HPCTOOLKIT}, etc.  In that case,
the easiest solution is to load the {\tt hpctoolkit} module.  Try
``{\tt module show hpctoolkit}'' to see if it sets {\tt HPCTOOLKIT}.
