...
than \verb|WALLCLOCK@1000|.  However, on IBM BlueGene, itimer is not
bound by the Hz rate and so sampling rates faster than 1,000 per
second are possible.

We recommend against using both PAPI and \verb|WALLCLOCK| events in the
...
\begin{description}
  \item{\verb|HPCTOOLKIT|} To function correctly, \hpcrun{} must know
       ...
       \verb|libexec| subdirectories. For most systems, the 
       installation procedure ensures that \hpcrun{} can find the requisite
       components. Some parallel job launchers, however, will \emph{copy} the
       \hpcrun{} script to a different location from the installed base. If your
  ...
  \item{\verb|HPCRUN_TMPDIR|} While profiling a program, \hpcrun{} must
       write some internal data to temporary files. By default, these files
       are written to \verb|/tmp|. On some systems, however, you may not
       ...
       where you have write permission. {\bf CAUTION:} Your home directory is \emph{not}
       a good choice, as some of \hpcrun{}'s temporary files can get large.

       NOTE: You do not strictly have to use \verb|HPCRUN_TMPDIR|. If you set the
       commonly-used Unix environment \verb|TMPDIR|, \hpcrun{} will use it for
       temporary files if \verb|HPCRUN_TMPDIR| is not set.
\end{description}

{\bf Note to system administrators:} if a module system is available, then constructing
an \HPCToolkit{} module to handle these environment variables would be convenient for
usres.
...
%
\subsection{Cray XE and XK}

Cray XE and XK systems have copying-style parallel job launchers referred
to in section~\ref{sec:env-vars}.
Consequently, when using dynamically linked binaries on Cray XE and XK systems, you
should add the \verb|HPCTOOLKIT| environment variable to your launch
script.  Set \verb|HPCTOOLKIT| to the top-level \HPCToolkit{} install
prefix (the directory containing the \verb|bin|, \verb|lib| and
\verb|libexec| subdirectories) and export it to the environment.  This is
