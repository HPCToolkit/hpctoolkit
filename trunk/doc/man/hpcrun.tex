...
\begin{Description}
  ...
  \item[\verb+HPCRUN_TMPDIR+] While profiling a program, \Prog{hpcrun} must
       write some internal data to temporary files. By default, these files
       are written to \File{/tmp}. On some systems, however, you may not
       have permission to write to \File{/tmp}. If this is the case on your system,
       ...
       a good choice, as some of \Prog{hpcrun}'s temporary files can get large.

       NOTE: You do not strictly have to use \verb+HPCRUN_TMPDIR+. If you set the
       commonly-used Unix environment \verb+TMPDIR+, \Prog{hpcrun} will use it for
