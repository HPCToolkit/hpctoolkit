...
\Prog{hpcview} correlates dynamic (`flat') profiles with static source code structure.  See \HTMLhref{hpctoolkit.html}{\Cmd{hpctoolkit}{1}} for an overview of \textbf{HPCToolkit}.
...
\Prog{hpcview} \oOpt{options} \Arg{config-file}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Description}

\Prog{hpcview} correlates dynamic profiling metrics with static source code structure and (by default) generates an Experiment database for use with \HTMLhref{hpcviewer.html}{\Cmd{hpcviewer}{1}}.
...
The easiest way to create a configuration file is to use the \HTMLhref{hpcquick.html}{\Cmd{hpcquick}{1}}, a front end to \Prog{hpcview}.
...
A \Prog{hpcview} configuration file is an XML document of type \texttt{HPCVIEW}.
...
There is one important difference in how each type is handled.  HPCRUN files are correlated to source code using the object code addresses annotations of STRUCTURE files; thus, they require valid STRUCTURE information.  In contrast, PROFILE files are correlated by source line level information.
  ...
  \item \textbf{Derived or COMPUTE metrics.} Derived metrics are specified by a \texttt{COMPUTE} element containing a MathML equation in terms of metrics defined earlier in the HPCVIEW document.  \Prog{hpcview} supports the following operands
  \begin{itemize}
