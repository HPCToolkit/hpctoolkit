...
\begin{Description}
\item[\OptoArg{--source}{=all,pgm,lm,f,p,l,s}] 
\item[\OptoArg{--src}{=all,pgm,lm,f,p,l,s}] 
Show metrics summarized by source structure. \{all\}
  \begin{itemize}
  \item all: all summaries below
...
\item[\Opt{--srcannot}] 
Annotate source files with metrics. Default path filter is `*'; change with \Opt{--srcfile}.

\item[\OptArg{--srcfile}{path-substring}] 
Annotate source files with path names that match \Arg{path-substring}.
...
Show a supplemental or different metric set. \Arg{metric} is one of the following:
  \begin{itemize}
  \item sum: Additionallly show Sum, Min, Max
  \item sum-only: Show only Sum, Min, Max
  \end{itemize}
...
\item[\OptoArg{--obj}{=s}] 
Show object code correlation: load modules, procedures, instructions. \{\}
  \begin{itemize}
